...
- **Default Assumption:** 25 minutes per complex PA (adjust per organization).
...
1. Generate data from the root project:
   ```bash
   cd ..  # repo root
   make batch-run      # or python -m priorauth.apps.agent.batch_runner
   ```
   This refreshes:
   - `apps/ui/public/dashboard_data.json` (decision log + metadata)
   - `apps/ui/public/governance_report.json` (FNR parity report)
   - `apps/ui/public/.last_model_trace.json` (TraceBanner telemetry)
