...
DATA_DIR = Path(os.getenv("PA_DATA_DIR", REPO_ROOT / "data")).resolve()
...
POLICY_PDF = Path(
    os.getenv("PA_POLICY_PDF", REPO_ROOT / "docs" / "assets" / "Policy_Weight_Mgmt_2025.pdf")
).resolve()
GUIDELINE_PATH = Path(
    os.getenv("PA_GUIDELINE_PATH", REPO_ROOT / "policies" / "source" / "UpdatedPAGuidelines.txt")
).resolve()
