...
from priorauth.policy_snapshot import (
    GUIDELINE_PATH,
    POLICY_ID,
    SNAPSHOT_PATH,
    canonical_dumps,
...
POLICY_PDF = paths.POLICY_PDF
EMBED_DIM_FILE = paths.CHROMA_DIR / "embedding_dim.txt"
...
def _ensure_embedding_dim(embedding_fn: OllamaEmbeddings, expected: int = 1024) -> int:
    ...
    if EMBED_DIM_FILE.exists():
        ...
        if recorded != dim or dim != expected:
            raise RuntimeError(
                ...
                f"Delete {paths.CHROMA_DIR} and rerun `python -m priorauth.apps.agent.setup_rag` to rebuild."
            )
    else:
    ...
    if dim != expected:
        raise RuntimeError(
            ...
            f"Delete {paths.CHROMA_DIR} and rerun `python -m priorauth.apps.agent.setup_rag`."
        )
    return dim
