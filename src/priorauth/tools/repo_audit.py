...
from priorauth.config import MAX_SEARCH_DEPTH
...
def _safe_find(root: str = str(paths.REPO_ROOT)) -> list[str]:
...
def _walk_limited(root: str = str(paths.REPO_ROOT), max_depth: int = MAX_SEARCH_DEPTH) -> list[str]:
    results: list[str] = []
...
def audit_repo() -> dict:
    ...
    try:
        found = _safe_find(str(paths.REPO_ROOT))
    except RuntimeError:
        found = _walk_limited(str(paths.REPO_ROOT), MAX_SEARCH_DEPTH)
