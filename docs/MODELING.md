...
- Embedding dimension guard: [src/priorauth/apps/agent/setup_rag.py](../src/priorauth/apps/agent/setup_rag.py) enforces 1024-dim embeddings and records the dimension in [chroma_db/embedding_dim.txt](../chroma_db/embedding_dim.txt).
- Chunking: not used. RAG documents are policy “atoms” constructed from structured policy sections in [src/priorauth/apps/agent/setup_rag.py](../src/priorauth/apps/agent/setup_rag.py).
...
Defaults come from [src/priorauth/config.py](../src/priorauth/config.py) and the Modelfiles in [models/](../models/).
...
If `PA_USE_RAW_MODELS=true`, the raw upstream models in [src/priorauth/config.py](../src/priorauth/config.py) are used instead of the custom Modelfiles.

## Scenario manifest fields
[output/scenario_manifest.json](../output/scenario_manifest.json) (also [tests/fixtures/baselines/v0_8/scenario_manifest.json](../tests/fixtures/baselines/v0_8/scenario_manifest.json)) includes:
...
## Benchmark evidence
The benchmark harness is implemented in [src/priorauth/apps/agent/benchmark.py](../src/priorauth/apps/agent/benchmark.py). For the latest recorded run, see [reports/benchmark.txt](../reports/benchmark.txt). RAG/rerank sanity evidence is tracked in [reports/rag_rerank_sanity.txt](../reports/rag_rerank_sanity.txt), and JSON parse notes live in [reports/llm_audit_json_debug.md](../reports/llm_audit_json_debug.md).
...
| LLM (qwen3) | 50 | 72674 ms | Not measured | Not measured | Deterministic avg 10.55 ms; agreement 50/50; advisory JSON parse warnings observed in prior qwen3 runs (see [reports/llm_audit_json_debug.md](../reports/llm_audit_json_debug.md)). |
