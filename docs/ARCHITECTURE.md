...
1.  **System 1 (Deterministic):** Fast, rule-based execution. Parses structured text, regex matches medical codes (ICD-10/LOINC), and enforces "Hard Stop" safety exclusions.
    *   *Implemented in:* [src/priorauth/policy_engine.py](../src/priorauth/policy_engine.py)
2.  **System 2 (Probabilistic):** Slow, deliberative reasoning. Synthesizes unstructured clinical notes, extracts intent, and maps complex patient histories to policy criteria.
    *   *Implemented in:* [src/priorauth/agent_logic.py](../src/priorauth/agent_logic.py) (Audit/Reasoning Nodes)
...
#### 1. Data Ingestion Layer ([src/priorauth/etl_pipeline.py](../src/priorauth/etl_pipeline.py), [src/priorauth/policy_snapshot.py](../src/priorauth/policy_snapshot.py))
*   **Responsibility**: Normalizing heterogeneous input data (CSV, Text) into canonical internal representations.
*   **Key Interaction**: Parses [policies/source/UpdatedPAGuidelines.txt](../policies/source/UpdatedPAGuidelines.txt) into a hashed JSON snapshot ([policies/RX-WEG-2025.json](../policies/RX-WEG-2025.json)) to serve as the **Single Source of Truth**.

#### 2. Retrieval Engine ([src/priorauth/apps/agent/setup_rag.py](../src/priorauth/apps/agent/setup_rag.py), [chroma_db/](../chroma_db/))
...
#### 3. The Agentic Core ([src/priorauth/agent_logic.py](../src/priorauth/agent_logic.py))
...
#### 4. The Safety Layer ([src/priorauth/policy_engine.py](../src/priorauth/policy_engine.py), [src/priorauth/governance_audit.py](../src/priorauth/governance_audit.py))
...
*   **Decision**: Implement a parallel, deterministic verification layer ([src/priorauth/policy_engine.py](../src/priorauth/policy_engine.py)) that runs *after* the LLM reasoning to validate the verdict.
*   **Trade-off**:
    *   *Pro*: Mathematically guaranteed safety for known exclusions.
    *   *Con*: Code duplication. Logic for "Pregnancy" exists in both the Policy text (for LLM) and Python constants (for Engine). mitigated via [src/priorauth/policy_utils.py](../src/priorauth/policy_utils.py) centralization.
...
*   **Decision**: Implement a text-to-JSON parser ([src/priorauth/policy_snapshot.py](../src/priorauth/policy_snapshot.py)) that reads a human-readable text file and compiles it into a machine-readable schema at runtime/build time.
*   **Trade-off**:
    *   *Pro*: Agility. Clinical Ops can update [policies/source/UpdatedPAGuidelines.txt](../policies/source/UpdatedPAGuidelines.txt) without touching code.
...
### Flow Sequence
1.  **Load**: [src/priorauth/apps/agent/batch_runner.py](../src/priorauth/apps/agent/batch_runner.py) lazy-loads Global Models (Embed, Rerank, LLM).
...
6.  **Persistence**: Results written to [output/](../output/) CSVs.
...
*   **Input Sanitization**: All terms are normalized via `policy_utils.normalize` (see [src/priorauth/policy_utils.py](../src/priorauth/policy_utils.py)) before matching.
...
*   **Type Safety**: Use of `Pydantic` models (`AuditResult` in [src/priorauth/agent_logic.py](../src/priorauth/agent_logic.py)) ensures LLM outputs strictly conform to required schemas (BMI as float, Enums for verdicts).
...
2.  **Async Worker Queues**:
    *   Decouple ingestion from processing. Use `Celery` + `Redis` to handle Authorizations asynchronously. The [src/priorauth/apps/agent/batch_runner.py](../src/priorauth/apps/agent/batch_runner.py) loop becomes a Producer; the Agent becomes a Consumer.
3.  **Model Serving**:
    ...
    *   The [src/priorauth/agent_logic.py](../src/priorauth/agent_logic.py) would call an HTTP endpoint for generation rather than invoking a local library.
